...
```{r}
#| label: tbl-one
#| tbl-cap: "Descriptive statistics by Midwest identity"
...
(a) The table, regression results, and figure, with appropriate captions (1 pt) This was done

(b) Inline R code in at least 2 places, 1 pulling a statistic from a table (i.e., using gtsummary::inline_text()) and 1 printing something else (like we did with the mean age in the example) (1 pt) There were `r nrow(nlsy_short)` respondents to the survey, with most participants responding `r most_common` to the question 'Do you consider yourself part of the Midwest?'. Among the respondents who responded `r most_common`, `r inline_text(tbl_one, variable = "gender", level = "Male", column = most_common, pattern = "{p}%")` were men.

(c) Cross-references to a table and a figure at least once each (1 pt) @fig-one contains a histogram and @tbl-one a table.
