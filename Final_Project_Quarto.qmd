...
(b) Inline R code in at least 2 places, 1 pulling a statistic from a table (i.e., using gtsummary::inline_text()) and 1 printing something else (like we did with the mean age in the example) (1 pt)
	There were `r nrow(nlsy_short)` respondents to the survey, with most participants responding `r names(which.max(table(nlsy$midwest_identity)))`. Among those who responded "Not at all" when asked if they identify as midwestern, most of them were women (`r inline_text(table1, variable = "Gender$Female", column = "Not at all")`).
...
```{r}
png(
  filename = here::here("plots", "age_distribution.png"),
  width = 800, 
  height = 600,
)
